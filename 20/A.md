...
$$ \max\_{x \ge 0} mex(\{s \oplus x \mid s \in S\}) $$
...
1.  **Плоские массивы (Flat Arrays)**:
    ...
    - Полностью исключены объекты узлов Trie.
    - Все данные Trie хранятся в четырех глобальных массивах `List<int>`: `l`, `r`, `cnt`, `memo`.
    - Это исключает нагрузку на Garbage Collector, который иначе тратил бы время на создание и очистку миллионов мелких объектов в процессе обновлений.

2.  **Ручной инлайнинг и zero-allocation**:

    - Все геометрические функции (pushUp, update) реализованы без возврата объектов.
