...
1.  **Плоские массивы (Flat Arrays)**:
    - Для таблицы чисел Стирлинга используется `List<List<int>>` с предварительным выделением памяти.
    - Массивы для степеней и посещений используют `List<int>` и `List<bool>` для эффективного доступа.
