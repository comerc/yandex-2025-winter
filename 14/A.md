...
1.  **Плоские массивы (Flat Arrays)**:
    - Не используются массивы, все вычисления выполняются с переменными.
...
1. **`solveFunc(a, q, L, R)`** — основная функция, обрабатывает все случаи
2. **`solveGeneral(a, q, L, R)`** — обрабатывает общий случай `|q| ≥ 2`
...
const int INV2 = 500000004; // 2^(-1) mod MOD
const int INV3 = 333333336; // 3^(-1) mod MOD
```
