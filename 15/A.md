...
1. **`solve(reader, writer)`** — основная функция, обрабатывает все тесты
2. **`processTestCase(s, t, writer)`** — обрабатывает один тест
3. **`checkLimits()`** — проверка ограничений времени и памяти (только при `CHECK_LIMITS`)
