...
1.  **Плоские массивы (Flat Arrays)**:
    - Метки хранятся в `Int32List` для координат (экономия памяти, так как координаты до 1e6).
    - Префиксные суммы в `Int64List` для избежания переполнения при суммах.

2.  **Ручной инлайнинг и zero-allocation**:
    - Все вычисления суммы расстояний реализованы без создания промежуточных объектов.
    - Бинарный поиск реализован вручную для максимальной скорости.
